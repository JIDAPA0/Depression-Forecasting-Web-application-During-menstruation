...
export function UserView() {
  ...
  const renderSymptomSection = (period: string, index: number) => {
    ...
    const physicalSymptoms = JSON.parse(symptomData.physicalSymptoms || '[]');
    const emotionalSymptoms = JSON.parse(symptomData.emotionalSymptoms || '[]');
    return (
      <Box key={index} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', mb: 3 }}>
        <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '100%', gap: 25 }}>
          <Box sx={{ flex: 1, textAlign: 'center' }}>
            ...
            {Array.isArray(physicalSymptoms) && physicalSymptoms.length > 0 ? (
              <ul>
                {physicalSymptoms.map((symptom: string, symptomIndex: number) => (
                  <li key={symptomIndex}>
          ...
          <Box sx={{ flex: 1, textAlign: 'center' }}>
            ...
            {Array.isArray(emotionalSymptoms) && emotionalSymptoms.length > 0 ? (
              <ul>
                {emotionalSymptoms.map((symptom: string, symptomIndex: number) => (
                  <li key={symptomIndex}>
                    <Typography variant='body2' sx={{ mb: 1, whiteSpace: 'nowrap' }}>
                      {symptom}
                    </Typography>
  ...
  return (
    ...
    >
            ...
            <Box key={index} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              {periodData ? (
                <>
                  <Typography variant="body2" sx={{ mb: 3 }}>
                    วันที่ประเมินล่าสุด:{" "}
                    {new Date(periodData.created_at).toLocaleDateString("th-TH", {
                      day: "numeric",
                      month: "long",
                      year: "numeric"
                      })}
      ...
      <Typography variant="body2" sx={{ mb: 2 }}>
        วันที่ประเมินล่าสุด :{" "}
        {psstData[0]
        ? new Date(psstData[0]?.created_at).toLocaleDateString("th-TH", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })
          ...
          {selectedDate && selectedDateAssessments && (
            <Box>
              <Typography sx={{ textAlign: "center", mb: 3 }}>
                วันที่:{" "}
                {new Date(selectedDate.getTime() - 86400000).toLocaleDateString("th-TH", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                  })}
